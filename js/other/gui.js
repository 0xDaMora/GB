...
function handleClick() {
	...
	var url = "https://ipfs.filebase.io/ipfs/Qmb3T32QSSSpJ3dyrRy2uy7YLmWwjw9zw2H2tqW5fLXnnr"; // reemplaza esto con tu URL
	request.open('GET', url, true);
	request.responseType = 'json';
	request.onload = function() {
		if (request.status >= 200 && request.status < 400) {
			var data = request.response;
			var datauri = data.data; // reemplaza esto con la clave de tu valor en el JSON
			if (datauri != null && datauri.length > 0) {
				try {
					cout(Math.floor(datauri.length * 3 / 4) + " bytes of data submitted by form (text length of " + datauri.length + ").", 0);
					initPlayer();
			...
			console.log('Error al obtener el archivo JSON: ' + request.status);
		}
	};
